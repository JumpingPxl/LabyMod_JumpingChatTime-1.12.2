...
public class FormatScreen extends OverlayScreen<String[]> {
	...
	public void initGui() {
		...
		textField = new TextField(mc.fontRenderer, centerX - 75, buttonY + 8, 150, 20, string -> {
			customValue = string;
		...
		for (FormatPlaceholder[] placeholders : PLACEHOLDERS) {
			...
			for (FormatPlaceholder placeholder : placeholders) {
				rowWidth += mc.fontRenderer.getStringWidth(placeholder.getFormat()) + 2;
			}

			buttonX = centerX - rowWidth / 2;
			for (FormatPlaceholder placeholder : placeholders) {
				PlaceholderButton placeholderButton = placeholder.createButton(mc.fontRenderer, textField,
						buttonX, buttonY);
