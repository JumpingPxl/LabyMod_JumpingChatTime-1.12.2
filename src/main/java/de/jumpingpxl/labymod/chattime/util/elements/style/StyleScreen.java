...
public class StyleScreen extends OverlayScreen<String> {
	...
	public void initGui() {
		...
		textField = new TextField(mc.fontRenderer, centerX - 78, y + 25, 156, 20, this::setValue);
		...
		for (String placeholder : PLACEHOLDERS) {
			...
			for (char character : charArray) {
				rowWidth += mc.fontRenderer.getStringWidth("&" + character) + 2;
			}

			if (placeholder.equals(PLACEHOLDERS[PLACEHOLDERS.length - 1])) {
				rowWidth += mc.fontRenderer.getStringWidth(TIME_PLACEHOLDER) + 2;
			}

			placeholderX = centerX - rowWidth / 2;
			for (char character : charArray) {
				PlaceholderButton placeholderButton = new PlaceholderButton(mc.fontRenderer,
		...
		PlaceholderButton placeholderButton = new PlaceholderButton(mc.fontRenderer, TIME_PLACEHOLDER,
				placeholderX, placeholderY, s -> textField.writeText(s));
		placeholderButtons.add(placeholderButton);
	}
